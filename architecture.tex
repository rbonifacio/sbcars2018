...
client-server architectural styles. The multi-tier architectural pattern 
...
the organization of a system using tiers supports software maintanance, facilitates the definition of 
...
description (see Figure~\ref{fig:runtime_qdm}). 

\begin{figure}
\includegraphics[scale=0.5]{img/runtimeView_qdm.png}
...
distribution of equipaments throughout the Brazilian Army, using her web browser, 
...
as a \emph{front controller} (in this case, a JSF controller). This controller validates  
...
is related to one or more \emph{military materials / equipaments} (MEMs) 
...
and has a series of actions (`then') that, in this case, trigger the distribution 
...
(and also for developers in future maintanance scenarios), we decided to use a 
...
SISDOT (Figure \ref{fig:runtime_qdm}), mostly because of its integration capabilities 
...
to implement this particular transformation (Figure \ref{fig:runtime_qdm}), using a template 
...
Therefore, the user-specified QC must be retrieved from the Brazilian Army's enterprise database (EB) 
...
The domain expet can then perform the necessary operations on the generated QDM, including a workflow 
...
declared. This is a time-consuming test, particularly when using the interface of 
...
using a Java test case requires 52 lines of imperative code.
...
We implemented our DSL using Xtext~\cite{eysholdt2010}, which generates plugins that allow editing code in bothe Eclipse and IntelliJ. 
...
test} is detailed as features using the Cucumber~\cite{wynne2017cucumber} framework. After that, the developer details 
the implementation steps of the features by (a) specifying which \callers (previously declaredusing our DSL) should be considered 
in the test execution, (b) specifying which QC the QDM should be generated, and (c) specifying the expected results in terms 
of the properties of the expected QDM. The structure created supports several operations, with the intention 
...
That is, our DSL simplifies the process of specifying \callers. Its primary goal was to 
...
to simulate the the specification of rules in order to better understand the effect of each 
\shc for building QDMs. Now we are working on a custom user interface for SISDOT to allow 
this specific feature. We are also working on the automatic generation of test cases 
from ou DSL. This effort involves a custom Maven plugin that currently generates simple test cases, 
...
organizational unities, randomly take 5 for generating the set of combinations), and the output directory 
