...
other areas of the logistic system~\cite{brasil2003}. Several rules govern the identification of materials and equipments, 
...
materials and equipments (hereafter MEMs) for the organizational unities of the Brazilian Army. Each organizational 
...
execution of test cases. In Sections~\ref{sec:rbs} and~\ref{sec:architecture} 
...
\section{Business Rules and Rule Based Engines}
\label{sec:rbs}
...
among others \cite{graham2007business}. This type of structure is very important for organizations, 
...
or if-then \cite{DBLP:books/daglib/0023820}.

RBS consist of two basic elements: a \emph{knowledge base} to store knowledge and an \emph{inference engine} that 
implements  algorithms to manipulate the knowledge represented in the knowledge base~\cite{grosan2011}, \cite{DBLP:books/daglib/0070547}, \cite{gallacher1989}.
The knowledge base stores facts and rules~\cite{DBLP:journals/cacm/Hayes-Roth85}. Fact is an usually static statement about properties, relations, 
...
is known as consequent, conclusion, action, or right-hand-side (RHS)~\cite{grosan2011}, \cite{abraham2005}. 
...
recently, languages such as Clojure, Ruby, Scala and Haskell adopted the use of template and quasi quote 
...
need to (re)compile the application~\cite{benato2017}, \cite{parr2006}.
